...
PortAudioDriver::PortAudioDriver(int sample_rate, int channels, int frame_duration_ms) 
...
// bool PortAudioDriver::initializeOpus() {
//     int error;
...
// void PortAudioDriver::cleanupOpus() {
//     if (encoder) {
//         opus_encoder_destroy(encoder);
//     }
//     if (decoder) {
...
bool PortAudioDriver::startRecording() {
...
bool PortAudioDriver::stopRecording() {
...
bool PortAudioDriver::getRecordedAudio(std::vector<int16_t>& recordedData) {
...
void PortAudioDriver::clearRecordedAudioQueue() {
    std::lock_guard<std::mutex> lock(recordedAudioMutex);
    std::queue<std::vector<int16_t>> empty;
    std::swap(recordedAudioQueue, empty);
}

int PortAudioDriver::recordCallback(const void *inputBuffer, void *outputBuffer,
    ...
    PortAudioDriver* audioProcess = static_cast<PortAudioDriver*>(userData);
...
bool PortAudioDriver::startPlaying() {
    if (isPlaying) {
...
bool PortAudioDriver::stopPlaying() {
    if (!isPlaying) {
...
int PortAudioDriver::playCallback(const void *inputBuffer, void *outputBuffer,
                               unsigned long framesPerBuffer,
                               const PaStreamCallbackTimeInfo* timeInfo,
                               PaStreamCallbackFlags statusFlags,
                               void *userData) {
...
void PortAudioDriver::clearPlaybackAudioQueue() {
    std::lock_guard<std::mutex> lock(playbackMutex);
    std::queue<std::vector<int16_t>> empty;
    std::swap(playbackQueue, empty);
}

void PortAudioDriver::addFrameToPlaybackQueue(const std::vector<int16_t>& pcm_frame) {
    std::lock_guard<std::mutex> lock(playbackMutex);
...
// std::queue<std::vector<int16_t>> PortAudioDriver::loadAudioFromFile(const std::string& filename, int frame_duration_ms) {
...
// void PortAudioDriver::saveToPCMFile(const std::string& filename, const std::queue<std::vector<int16_t>>& audioQueue) {
...
// void PortAudioDriver::saveToPCMFile(const std::string& filename) {
//     std::unique_lock<std::mutex> lock(recordedAudioMutex);
//     saveToPCMFile(filename, recordedAudioQueue);
// }

// bool PortAudioDriver::encode(const std::vector<int16_t>& pcm_frame, uint8_t* opus_data, size_t& opus_data_size) {
//     if (!encoder) {
...
// bool PortAudioDriver::decode(const uint8_t* opus_data, size_t opus_data_size, std::vector<int16_t>& pcm_frame) {
//     if (!decoder) {
...
// BinProtocol* PortAudioDriver::PackBinFrame(const uint8_t* payload, size_t payload_size) {
...
// bool PortAudioDriver::UnpackBinFrame(const uint8_t* packed_data, size_t packed_data_size, BinProtocolInfo& protocol_info, std::vector<uint8_t>& opus_data) {
    // 检查输入数据的有效性
